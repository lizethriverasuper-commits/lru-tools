...
const DEFAULT_LOGO = document.getElementById("loginLogo") ? document.getElementById("loginLogo").getAttribute("src") : "";
...
async function toggleRecipeActive(clientId, recipeId, onDone) {
  const list = STATE.recipes[clientId] || [];
  const r = list.find(x => x.id === recipeId);
  if (!r) return;
  r.active = r.active === false ? true : false;
  await saveState();
  toast(r.active === false ? "Receta dada de baja" : "Receta reactivada");
  if (onDone) onDone();
}

function deleteRecipeWithConfirm(clientId, recipeId, onDone) {
  const list = STATE.recipes[clientId] || [];
  const r = list.find(x => x.id === recipeId);
  if (!r) return;
  confirmModal(
    "Eliminar receta",
    `"${r.nombre}" se eliminará de forma permanente, incluida su foto. Esta acción no se puede deshacer. ¿Continuar?`,
    async () => {
      STATE.recipes[clientId] = list.filter(x => x.id !== recipeId);
      await saveState();
...
function renderSavedList() {
  ...
  const showBaja = document.getElementById("savedShowBaja")?.checked;

  let recipes = allRecipes.filter(r => showBaja ? r.active === false : r.active !== false);
  if (query) recipes = recipes.filter(r => r.nombre.toLowerCase().includes(query));
  ...
  if (recipes.length === 0) {
    list.innerHTML = `<div class="empty-state">${showBaja ? "No tienes recetas dadas de baja." : "No se encontró ninguna receta con ese nombre."}</div>`;
    return;
  }
  list.innerHTML = "";
  recipes.slice().reverse().forEach(r => {
    const item = document.createElement("div");
    item.className = "saved-item" + (r.active === false ? " is-baja" : "");
    ...
    item.innerHTML = `
      <div style="display:flex; align-items:center; flex:1; min-width:0;">
        ${thumb}
        <div style="min-width:0;">
          <div class="si-name">${esc(r.nombre)} ${r.active === false ? '<span class="tag tag-pend" style="margin-left:6px;">De baja</span>' : ''}</div>
...
function downloadFullBackup() {
  const backup = {
    ...
    exportedBy: "LRU Consultoría - Panel Admin",
    clients: STATE.clients,
    recipes: STATE.recipes,
    logos: STATE.logos.map(l => ({ id: l.id, name: l.name })), // sin dataUrl, para no pesar de más
  };
...
async function createClient() {
  ...
  const isBM = name.trim().toLowerCase() === "barra maretazo";
  // Barra Maretazo mantiene siempre su navy/gold, sin importar lo que se haya elegido en el selector
  const colorMain = isBM ? "#1B2A4A" : (document.getElementById("newClientColorMain").value || "#152D3D");
  const colorAccent = isBM ? "#B8862B" : (document.getElementById("newClientColorAccent").value || "#9A7B4F");

  const newClientId = uid("cli");
  STATE.clients.push({ id: newClientId, name, pin, active: true, createdAt: new Date().toISOString(), logoId: null, colorMain, colorAccent });
  ...
  document.getElementById("newClientColorMain").value = "#152D3D";
  document.getElementById("newClientColorAccent").value = "#9A7B4F";
  renderClientList();
  renderRecetaClientSelect();
  toast(isBM ? "Cliente creado con 54 platos precargados y su paleta navy/gold ✓" : "Cliente creado ✓");
}
...
function renderRecetaListAdmin() {
  ...
  const showBaja = document.getElementById("adminShowBaja")?.checked;
  ...
  if (recipes.length === 0) {
    el.innerHTML = `<div class="empty-state">${showBaja ? "No hay recetas dadas de baja." : "No se encontró ninguna receta con ese nombre."}</div>`;
    return;
  }
  el.innerHTML = recipes.map(r => `
    <div class="recipe-admin-item">
      <div>
        <div class="ra-name">${esc(r.nombre)} ${r.active === false ? '<span class="tag tag-pend" style="margin-left:6px;">De baja</span>' : ''}</div>
      ...
      <div class="cc-actions">
        ...
        <button class="mini-btn" data-radm="toggle" data-id="${r.id}">${r.active === false ? 'Reactivar' : 'Dar de baja'}</button>
        <button class="mini-btn" data-radm="delete" data-id="${r.id}" style="color:#b3402f;">Eliminar</button>
      </div>
    </div>
  `).join("");
  ...
  el.querySelectorAll("[data-radm]").forEach(btn => {
    btn.onclick = () => {
      ...
      else if (btn.dataset.radm === "toggle") toggleRecipeActive(clientId, r.id, renderRecetaListAdmin);
      else if (btn.dataset.radm === "delete") deleteRecipeWithConfirm(clientId, r.id, renderRecetaListAdmin);
      else exportRecipePdf(r, clientId);
    };
  });
}

function exportRecipePdf(r, clientId) {
  const client = STATE.clients.find(c => c.id === clientId);
  const colorMain = (client && client.colorMain) || "#152D3D";
  const colorAccent = (client && client.colorAccent) || "#9A7B4F";
  ...
  win.document.write(`
    ...
    <style>
      ...
      body{font-family:'Helvetica Neue',Arial,sans-serif; color:${colorMain};}
      .r-head{border-bottom:3px solid ${colorAccent}; padding-bottom:10px; margin-bottom:16px; display:flex; align-items:center; gap:12px;}
      .r-head img{width:36px; height:36px; border-radius:50%;}
      .r-head h1{font-size:22px; color:${colorMain};}
      .r-meta{color:#6b7080; font-size:12px; margin-bottom:14px;}
      .r-section-title{font-size:13px; text-transform:uppercase; letter-spacing:1px; color:${colorAccent}; font-weight:800; margin:18px 0 8px; border-left:4px solid ${colorAccent}; padding-left:8px;}
      table{width:100%; border-collapse:collapse; font-size:12px;}
      th{background:${colorMain}; color:#fff; padding:6px 8px; text-align:left; font-size:10.5px; text-transform:uppercase;}
      td{padding:6px 8px; border-bottom:1px solid #e3e2dc;}
      .paso{margin-bottom:8px; font-size:12.5px;}
      .paso b{color:${colorMain};}
      .notas{background:#FFFAF0; border:1px solid ${colorAccent}; border-radius:6px; padding:10px 14px; font-size:12px; margin-top:10px;}
    </style></head><body>
...
function exportRecetarioBrandPdf() {
  ...
  const colorMain = client.colorMain || "#152D3D";
  const colorAccent = client.colorAccent || "#9A7B4F";
  const logoId = document.getElementById("logoForExportSelect").value;
  const logo = STATE.logos.find(l => l.id === logoId);
  const logoUrl = logo ? logo.dataUrl : DEFAULT_LOGO;

  const win = window.open("", "_blank");
  const pages = recipes.map(r => recipePageHTML(r, colorMain, colorAccent)).join('<div style="page-break-after:always;"></div>');
  ...
  win.document.write(`
    ...
    <style>
      ...
      .cover h1{font-size:34px; color:${colorMain}; margin-bottom:6px;}
      .cover h2{font-size:18px; color:${colorAccent}; font-weight:600;}
      .r-head{border-bottom:3px solid ${colorAccent}; padding-bottom:10px; margin-bottom:16px; display:flex; align-items:center; gap:12px;}
      .r-head img{width:36px; height:36px; border-radius:50%;}
      .r-head h1{font-size:22px; color:${colorMain};}
      .r-meta{color:#6b7080; font-size:12px; margin-bottom:14px;}
...
function recipePageHTML(r, colorMain, colorAccent) {
...
async function init() {
  ...
  document.getElementById("savedShowBaja")?.addEventListener("change", () => renderSavedList());
  document.getElementById("adminShowBaja")?.addEventListener("change", () => renderRecetaListAdmin());
